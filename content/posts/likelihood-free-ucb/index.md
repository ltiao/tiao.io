...
The readouts above share a premise worth making explicit: each is a functional of the predictive distribution at a single point. PI, EI, UCB, the entropic score, the quantile — all of them ask what $p(y \mid \mathbf{x})$ says about $\mathbf{x}$ and nothing else. That is the precondition for the whole likelihood-free program, because raw pairs $(\mathbf{x}_i, y_i)$ are samples from exactly those marginals, and functionals of the marginals are the things a loss can hope to elicit.

The rest of the acquisition zoo lives on the other side of that line. The knowledge gradient[^frazier2018] scores an observation by how much it improves the maximum of the posterior mean over the whole domain; entropy search and its descendants[^entropysearch] score it by the mutual information between $y(\mathbf{x})$ and the location (or value) of the global optimum. Every one of these is a functional of the posterior over $f$, the joint object with its correlation structure, rather than of any single marginal. Two models can agree on every predictive $p(y \mid \mathbf{x})$ and still disagree about KG and ES at every point, because they disagree about how an observation here propagates to beliefs there. For these acquisitions the likelihood-free wall is not elicitability but ontology: there is no loss to design, because the quantity being scored (a change in posterior knowledge) does not exist until a posterior is posited, and refusing to posit one is the program's founding move. The catch was never really about quantile regression. Value-of-information acquisitions are epistemic all the way down.

What the marginal world retains are the residues. Sever every cross-input correlation in the posterior, so that observing $y(\mathbf{x})$ informs only $\mathbf{x}$ itself, and the knowledge gradient collapses in two lines: the post-observation maximum of the mean is $\max(\mu^{\ast}, m(y))$, where $m(y)$ is the updated mean at $\mathbf{x}$ and $\mu^{\ast}$ the incumbent maximum, so $\mathrm{KG}(\mathbf{x}) = \mathbb{E}\left[ \max(m(y) - \mu^{\ast}, 0) \right]$: expected improvement, computed on the distribution of the updated mean. And severing the same correlations in Thompson sampling, so that each candidate's value is drawn from its own marginal rather than all values from one function draw, produces exactly the *independent* Thompson sampling that the coda below dismantles. The adjective was doing the work all along. Cut the joint, and every posterior acquisition falls back into the CDF world: KG lands on EI, and Thompson sampling, as we are about to see, lands on UCB.
